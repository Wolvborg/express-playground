...
const $CART_FILE = path.join(require.main.path, "data", "cart.json");

const $PRODUCT_FILE = path.join(require.main.path, "data", "products.json");
    ...
    return new Promise((resolve, reject) => {
      getDataFromFile($CART_FILE, (data) => {
        if (data) {
          ...
          if (Object.keys(cartObj).length) resolve(cartObj);
          else {
      ...
      getDataFromFile($CART_FILE, (cart_file_data) => {
        if (cart_file_data) {
          ...
          if (Object.keys(cartObj).length === 0) {
            ...
            getDataFromFile($PRODUCT_FILE, (product_file_data) => {
              let existingProductIndex = cartObj.products.findIndex(
                (product) => product.id === productId
              );
              if (existingProductIndex > -1) {
                ...
                set(
                  cartObj,
                  `products[${existingProductIndex}].qty`,
                ...
                set(
                  cartObj,
                  "total_amount",
                  cartObj.total_amount + existingProduct.price
                );
              } else {
                let products = JSON.parse(product_file_data);
                ...
                set(
                  cartObj,
                  "total_amount",
                  cartObj.total_amount + product.price
                );
              }
              putDataIntoFile($CART_FILE, JSON.stringify(cartObj), (err) => {
                if (err) reject(err);
  ...
  removeFromCart: function (id) {
    ...
    return new Promise((resolve, reject) => {
      getDataFromFile($CART_FILE, (cart_file_data) => {
        if (cart_file_data) {
          let cartObj = JSON.parse(cart_file_data);
          let productToBeDeletedIndex = cartObj.products.findIndex(
            (product) => product.id === productId
          );
          ...
          set(
            cartObj,
            "total_items",
            cartObj.total_items - productToBeDeleted.qty
          );
          set(
            cartObj,
            "total_amount",
            cartObj.total_amount -
              productToBeDeleted.qty * productToBeDeleted.price
          );
          putDataIntoFile($CART_FILE, JSON.stringify(cartObj), (err) => {
            if (err) reject(err);
            else resolve();
